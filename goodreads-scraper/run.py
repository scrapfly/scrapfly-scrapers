...
It scrapes book and list data and saves it to ./results/
...
async def run():
    ...
    output.joinpath("book.json").write_text(json.dumps(book, indent=2, ensure_ascii=False))
    ...
    output.joinpath("list.json").write_text(json.dumps(book_list, indent=2, ensure_ascii=False))
